...
for filename in os.listdir(models_availability_directory):
    if filename.endswith(".joblib"):
        ...
        models_availability[station_number] = joblib.load(
            os.path.join(models_availability_directory, filename)
        )
...
for filename in os.listdir(models_bike_stands_directory):
    if filename.endswith(".joblib"):
        ...
        # Load the model and store it in the dictionary
        models_bike_stands[station_number] = joblib.load(
            os.path.join(models_bike_stands_directory, filename)
        )
...
def get_availability():
    ...
    with connection:
        with connection.cursor() as cursor:
            cursor.execute(
                f"SELECT * FROM dbikes.availability ORDER BY curr_time DESC LIMIT {station_count};"
            )
            rows = cursor.fetchall()
            row_count = 0
            for row in rows:
...
def predict():
    try:
        ...
        temp_c, feels_like_c, wind_speed, humidity = process_weather_forecast(
            unix_timestamp
        )
